...
class ImageProcessor(ImageProcessorInterface):
    ...
    def compute_camera_point_cloud(self, points_2d, d, n):
        ...
        points_undistorted = cv2.undistortPoints(points_for_undistort, np.asarray(self.config.calibration.camera_matrix), np.asarray(self.config.calibration.distortion_vector))
